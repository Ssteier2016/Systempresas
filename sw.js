// sw.js - Service Worker para Gestión de Negocios Pro v3.9.16 (Full Experimental + Chrome & CORS Fix)
const CACHE_NAME = 'business-app-v3.9.16';
const urlsToCache = [
  '/',
  '/index.html',
  '/manifest.json',
...
// INSTALACIÓN RESILIENTE (Ignora fallos de CORS o red en archivos individuales)
self.addEventListener('install', event => {
  console.log('[Service Worker] Instalando v3.9.16...');
  event.waitUntil(
    caches.open(CACHE_NAME).then(cache => {
      return Promise.allSettled(
        urlsToCache.map(url => {
          return fetch(url, { mode: url.includes('http') ? 'cors' : 'no-cors' }).then(res => {
...
// ESTRATEGIA FETCH CON FIX DE PROTOCOLO CRÍTICO REFORZADO
self.addEventListener('fetch', event => {
  const url = event.request.url;

  // --- SOLUCIÓN DEFINITIVA AL ERROR DE CHROME-EXTENSION ---
  if (!url.startsWith('http')) return;

  // Excluir peticiones dinámicas de Firebase/Google APIs
    ...
    event.respondWith(
      fetch(event.request).then(response => {
        if (response.status === 200) {
          ...
          // Verificación de esquema antes de guardar
  ...
  // Lógica Cache First para el resto de recursos
  event.respondWith(
    caches.match(event.request).then(cached => {
      if (cached) return cached;
      return fetch(event.request).then(response => {
        // No cachear si la respuesta no es exitosa o es inválida
        if (!response || response.status !== 200 || response.type !== 'basic') return response;
        
        const copy = response.clone();
        // Doble validación de protocolo antes del put
        if (event.request.url.startsWith('http')) {
          caches.open(CACHE_NAME).then(cache => cache.put(event.request, copy));
        }
        return response;
